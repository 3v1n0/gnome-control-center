...
connect_ip6_page (CEPageIP6 *page)
{
        ...
        radios[1] = GTK_TOGGLE_BUTTON (gtk_builder_get_object (CE_PAGE (page)->builder, "radio_dhcp"));
...
ui_to_setting (CEPageIP6 *page)
{
        ...
        if (g_str_equal (method, NM_SETTING_IP6_CONFIG_METHOD_AUTO) ||
            ...
            g_str_equal (method, NM_SETTING_IP6_CONFIG_METHOD_MANUAL))
                dns_adresses = g_strsplit (dns_text, ", ", -1);
        else
                dns_adresses = NULL;

        for (i = 0; dns_adresses && dns_adresses[i]; i++) {
                const gchar *text;
                struct in6_addr tmp_addr;

                text = g_strstrip (dns_adresses[i]);
                if (!*text) {
                        /* ignore empty rows */
        ...
        g_clear_pointer (&dns_adresses, g_strfreev);
